...
This note describes an example of driver analysis based on the random forest model
and SHAP values.
...
(short names used for output are in parentheses):

1. It is a healthy snack (health)
2. Informs transparently its consumers (transparency)
3. Listens to its consumers (listens)
4. Has a good reputation (reputation)
5. Is a brand that acts responsibly (responsible)
6. Is committed to sustainability (sustainability)

Moreover, to control for the brand effect two scales were used:

7. respondent's attachment to brand (attachement)
8. respondent's familiarity with brand (familiarity)
...
#set short names for drivers (instead of numbers)
colnames(data) <- c(colnames(data)[1:4], drivers_names[, 2])
```
...
* *Attachment* and *Familiarity* have bigger number of values than remaining drivers.
* Although remaining drivers have fairly similar means and standard deviations,
their distributions show noticeable differences.
  * *Health* has a relatively high share of values = 4.
  * *Reputation* has a relatively high share of values = 5.
  * *Sustainability* has a relatively high share of values = 3, consequently a lower mean
  than others.
* In *Attachment's* distribution dominate values close to the middle of the scale,
while *Familiarity's* distribution has relatively more high values.
...
* Drivers *Attachment* and *Familiarity* are relatively less correlated with both 
"trust" and other drivers.
* Correlations are stronger for brand "B" than for brand "A".
* Correlations are stronger for Germany than for France and Sweden.
...
For an ensemble of regression trees, a mean increase in MSE when a feature is not present is typically used. However, drivers 1-6 and drivers 7-8  (i.e., Attachment, Familiarity) have a different number of classes. It may lead to an upward bias in estimating the importance of drivers with a larger number of classes. To avoid this issue, a modified Gini impurity measure (see Nembrini et al. (2018)) is used.
...
#tuneRanger does not allow for importance = "impurity_corrected"
task <- makeRegrTask(id = "a", data = data_rf, target = "trust")
tuned <- tuneRanger(
  task,
  measure = list(mse),
  num.trees = 1000,
  parameters = list(replace = T, importance = "impurity"),
  iters = 10
)
#select a subset of best performing combinations of hyperparameters
...
1. A two-way plot contrasting a mean driver score (performance) with mean absolute SHAP value (importance).
...
2. Nonlinearities detection for drivers.
