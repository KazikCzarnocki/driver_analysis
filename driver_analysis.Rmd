...
#only brand A
cor_matrix <- cor(data %>% filter(brand == "A") %>% select(-c(ID, brand, country)))
corrplot(
  cor_matrix,
  type = "full",
  order = "original",
...
#only brand B
cor_matrix <- cor(data %>% filter(brand == "B") %>% select(-c(ID, brand, country)))
corrplot(
  cor_matrix,
  type = "full",
  order = "original",
...
#only country DE
cor_matrix <- cor(data %>% filter(country == "DE") %>% select(-c(ID, brand, country)))
corrplot(
  cor_matrix,
  type = "full",
  order = "original",
...
#only country FR
cor_matrix <- cor(data %>% filter(country == "FR") %>% select(-c(ID, brand, country)))
corrplot(
  cor_matrix,
  type = "full",
  order = "original",
...
#only country SE
cor_matrix <- cor(data %>% filter(country == "SE") %>% select(-c(ID, brand, country)))
corrplot(
  cor_matrix,
  type = "full",
  order = "original",
...
best_parameters <- tuned$results %>%
  ...
  arrange(., mse)
#To find the best stable solution lets estimate each of these models 100 times 
...
```{r echo=FALSE}
#mean driver score
#because drivers have different ranges, we need to scale them
#otherwise they would not be comparable on a common scale
mean_value <- data %>%
  ...
  as.data.frame() %>%
  rownames_to_column()
colnames(mean_value) <- c("driver", "mean_value")

#mean absolute SHAP value
#they are scaled as a percentage of their sum
mean_shap <- shap$shaps[,-1] %>%
  mutate(brand = data$brand,
